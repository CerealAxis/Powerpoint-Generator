...
7. **补充信息**（自由文本，以下为提示项）：
   ...
   - 视觉风格偏好（如公司有品牌规范）
    - **配图需求**：
      ...
      2. 图片从哪里来？（如选"不需要配图"则跳过此问）
          - A. 用户提供图片（请附上图片路径或直接发送图片文件）
          - B. AI 生成（调用 image_generate 工具）
          - C. Unsplash 图库（需要配置 UNSPLASH_ACCESS_KEY 环境变量）

      **降级说明**：如选择 B 或 C，当该方式不可用时会自动降级，无需再次询问。

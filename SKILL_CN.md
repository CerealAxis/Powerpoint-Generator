...
5. **智能配图** -- 利用图片生成能力（AI生成或unsplash获取）为每页配插图
...
| 规模 | 页数 | 调研 | 搜索 | 策划 | 生成                      |
|------|------|------|------|------|-------------------------|
| **轻量** | <= 8 页 | 3 题精简版（场景+受众+补充信息） | 3-5 个查询 | Step 3 可与 Step 4 合并一步完成 | 每页并行生成                  |
| **标准** | 9-18 页 | 完整 7 题 | 8-12 个查询 | 完整流程 | 每页并行生成      |
| **大型** | > 18 页 | 完整 7 题 | 10-15 个查询 | 完整流程 | 每页并行生成 |
...
1. 搜索主题背景资料（3-5 条）
2. 根据复杂度选择完整 7 题或精简 3 题，一次性发给用户，同时告知用户支持的风格选项供其选择（16 种预设风格）
3. **等待用户回复**（阻断点）
4. 整理为需求 JSON
...
| 执行层 | 7. 补充信息（演讲人/品牌色/必含/必避/页数/配图偏好/PPT风格） | 具体执行细节 |
...
**⚠️ 环境要求**: 需要 Python ≥ 3.8（`Tuple` 类型注解）。若 Python 版本过低导致导入失败，请告知用户需升级 Python 或在支持的环境中运行。
